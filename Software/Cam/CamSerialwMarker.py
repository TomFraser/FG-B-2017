...
fieldThresholds = [(36, 88, 25, 76, 13, 63), #Update for thailand colours on arival <=---==-=-=-=-
(36, 88, 25, 76, 13, 63),
(36, 88, 25, 76, 13, 63),
(36, 88, 25, 76, 13, 63)]
...
while(True):
    ...
    # field Markers
    for fieldmarkersNum in fieldThresholds:
        fieldMarker = img.find_blobs([fieldThresholds[fieldmarkersNum]], x_stride=10, y_stride=10, area_threshold=50, pixel_threshold=50, merge=True)
        if(len(fieldMarker)):
            fieldMarker = sorted(fieldMarker, key= lambda goal: goal.pixels(), reverse=True)[0]

            if(sqrt(fieldMarker.pixels())>10):
                x = -(fieldMarker.cx() - (img.width() / 2))
                y = fieldMarker.cy() - (img.height() / 2)
                fieldMarkerAngle = (atan2(y,x) * (180 / pi) - 90)%360
                fieldMarkerSize = 2*(sqrt(x**2 + y**2))
                #print(fieldMarkerSize)
                img.draw_rectangle(fieldMarker.rect())
                img.draw_cross(fieldMarker.cx(), fieldMarker.cy())
