...
void DirectionController::calcMotors(double angle, double lightAngle, double rotation){
    ...
    if(lightAngle != 0){
        motorA.set((cos(((angleArray[0] + 90) * angToRad) - (lightAngle * angToRad))) * (SPEED_VAL*2.55) + (int)rotation); //Light Movement
        motorB.set((cos(((angleArray[1] + 90) * angToRad) - (lightAngle * angToRad))) * (SPEED_VAL*2.55) + (int)rotation);
        motorC.set((cos(((angleArray[2] + 90) * angToRad) - (lightAngle * angToRad))) * (SPEED_VAL*2.55) + (int)rotation);
        motorD.set((cos(((angleArray[3] + 90) * angToRad) - (lightAngle * angToRad))) * (SPEED_VAL*2.55) + (int)rotation);
    }else{
        if(angle != 65506.00){
            motorA.set((cos(((angleArray[0] + 90) * angToRad) - (angle * angToRad))) * (SPEED_VAL*2.55) + (int)rotation); //Probs should do this motor stuff in the main application? I guess we can do it here tho. Might be less clear to observers
            motorB.set((cos(((angleArray[1] + 90) * angToRad) - (angle * angToRad))) * (SPEED_VAL*2.55) + (int)rotation);
            motorC.set((cos(((angleArray[2] + 90) * angToRad) - (angle * angToRad))) * (SPEED_VAL*2.55) + (int)rotation);
            motorD.set((cos(((angleArray[3] + 90) * angToRad) - (angle * angToRad))) * (SPEED_VAL*2.55) + (int)rotation);
        }else{
            motorA.set(0 + (int)rotation);
            motorB.set(0 + (int)rotation);
            motorC.set(0 + (int)rotation);
            motorD.set(0 + (int)rotation);
        }
    }
}
